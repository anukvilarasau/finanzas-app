...
export default function Dashboard({ income, expenses, budgetRules, setIncome, setBudgetRules }) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-5">
      ...
      {/* Income prompt */}
      {income === 0 && (
        <div className="border border-zinc-200 rounded-xl p-4 flex items-center gap-4 bg-white shadow-sm">
          <AlertCircle className="text-zinc-400 shrink-0" size={18} />
          <div className="flex-1">
            <p className="text-black font-mono text-sm font-semibold">income not configured</p>
            <p className="text-zinc-400 text-xs font-mono mt-0.5">Set your monthly income to unlock financial analysis.</p>
          </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <StatCard label="gastos_mes" value={fmt(analysis.total)} sub={income > 0 ? `${((analysis.total/income)*100).toFixed(0)}%_ingreso` : '—'} icon={<TrendingDown className="text-zinc-400" size={15} />} valueColor="text-black" />
        <StatCard label="ahorro_mes" value={income > 0 ? fmt(analysis.savings) : '—'} sub={income > 0 ? `${analysis.savingsPct.toFixed(0)}%_ingreso` : '—'} icon={<TrendingUp className={savingsOk ? 'text-black' : 'text-zinc-300'} size={15} />} valueColor={savingsOk ? 'text-black' : 'text-zinc-400'} />
        <StatCard label={`meta_ahorro_${savingsPct}%`} value={income > 0 ? fmt(savingsTarget) : '—'} sub={income > 0 ? (savingsOk ? 'TARGET_MET ✓' : `deficit: ${fmt(Math.max(0, savingsTarget - analysis.savings))}`) : '—'} icon={<DollarSign className={savingsOk ? 'text-black' : 'text-zinc-300'} size={15} />} valueColor={savingsOk ? 'text-black' : 'text-zinc-400'} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white border border-zinc-200 rounded-xl p-5 shadow-sm">
          <p className="text-xs text-zinc-400 font-mono uppercase tracking-widest mb-4">// recommendations</p>
          <div className="space-y-3">
            {recommendations.map((rec, i) => (
              <div key={i} className="flex items-start gap-3 text-sm text-zinc-700 leading-relaxed">
                <span className="text-zinc-300 font-mono text-xs shrink-0 mt-0.5">{String(i+1).padStart(2,'0')}.</span>
                <span>{rec}</span>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white border border-zinc-200 rounded-xl p-5 shadow-sm">
          <p className="text-xs text-zinc-400 font-mono uppercase tracking-widest mb-4">// by_category</p>
            ...
            <div className="flex items-center gap-4">
              <ResponsiveContainer width={120} height={120}>
                <PieChart>
                  ...
                  <Tooltip formatter={v => [fmt(v), 'Total']} contentStyle={{ background: '#fff', border: '1px solid #e4e4e7', borderRadius: '8px', color: '#000', fontSize: '11px', fontFamily: 'monospace' }} />
                </PieChart>
              </ResponsiveContainer>
              <div className="flex-1 space-y-1.5">
                {categoryTotals.map((cat, i) => (
                  <div key={i} className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-black font-mono ml-2 shrink-0">{fmt(cat.value)}</span>
                  </div>
                ))}
              </div>
            </div>
    ...
    <div className="space-y-1.5">
      <div className="flex justify-between items-center text-xs">
        ...
        <div className="flex items-center gap-2">
          {tracked ? (
            <>
              <span className={`font-mono font-semibold ${over && !isInverse ? 'text-red-500' : 'text-black'}`}>{fmt(actual)}</span>
              <span className="text-zinc-300 font-mono">/ {fmt(recommended)}</span>
              {ok ? <CheckCircle size={11} className="text-black" /> : <AlertCircle size={11} className="text-red-500" />}
            </>
          ) : (
            <span className="text-zinc-400 font-mono">target: {fmt(recommended)}</span>
          )}
        </div>
      </div>
      <div className="h-1.5 bg-zinc-100 rounded-full overflow-hidden">
        <div className="h-full rounded-full transition-all duration-700" style={{ width: tracked ? `${pct}%` : '0%', backgroundColor: barColor }} />
      </div>
    </div>
  )
}
